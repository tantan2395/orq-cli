"""Local IPC helpers for communication between Orchestrator Engine and MCP stdio clients."""
...
class IPCServer:
    """Async Unix Domain Socket server running in the Orchestrator Engine."""
    ...
    async def _handle_client(
        ...
        """Processes incoming requests from MCP client bridge."""
        while True:
...
class IPCClient:
    """Client used by the MCP stdio bridge to talk to the running Orchestrator Engine."""
    ...
    async def call(self, method: str, params: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not self.socket_path.exists():
            return {
                "status": "error",
                "error": f"Orchestrator IPC socket not found at {self.socket_path}. Is the TUI engine running?",
            }
